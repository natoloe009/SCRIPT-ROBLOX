...
scrollThumbFrame.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.UserInputType == Enum.UserInputType.Touch then
        ...
        if touchPos[dir] >= (scrollThumb.AbsolutePosition[dir] + scrollThumb.AbsoluteSize[dir]) then
            ScrollDirection = 1
        end

        local function doTick()
            local scrollSize = self.VisibleSpace - 1
            local incrementValue = type(self.Increment) == "number" and self.Increment or 1
            
            -- Debug prints
            print("ScrollDirection:", ScrollDirection)
            print("touchPos[dir]:", touchPos[dir], type(touchPos[dir]))
            print("scrollThumb.AbsolutePosition[dir]:", scrollThumb.AbsolutePosition[dir], type(scrollThumb.AbsolutePosition[dir]))
            print("scrollThumb.AbsoluteSize[dir]:", scrollThumb.AbsoluteSize[dir], type(scrollThumb.AbsoluteSize[dir]))
            print("incrementValue:", incrementValue, type(incrementValue))
            
            if ScrollDirection == 0 and touchPos[dir] < scrollThumb.AbsolutePosition[dir] then
                self:ScrollTo(self.Index - (incrementValue * 5))
            elseif ScrollDirection == 1 and touchPos[dir] >= scrollThumb.AbsolutePosition[dir] + scrollThumb.AbsoluteSize[dir] then
                self:ScrollTo(self.Index + (incrementValue * 5))
            end
        end
        ...
        -- When touch input ends
        local Connection = UserInputService.InputEnded:Connect(function(inputEnded)
        ...
        self.ScrollBDown = tick()
        while self.ScrollBDown do
            task.wait()
            if (tick() - self.ScrollBDown) >= 0.3 then
                doTick()
            end
        end
